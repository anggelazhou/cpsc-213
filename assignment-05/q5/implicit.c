...
static void *coalesce(struct heap *h, void *first_block_start) {
  ...
  if (is_within_heap_range(h, next_block)) {
    if (!block_is_in_use(next_block)) {
      // merge
       int block_size = get_block_size(first_block_start);
       int merged_size = block_size + get_block_size(next_block);
       set_block_header(first_block_start, merged_size, 0);
    }
  }
...
static int get_size_to_allocate(int user_size) {
  int total_size = user_size + 2*HEADER_SIZE;
  int remainder = total_size % 4;
  int aligned_size = remainder == 0 ? total_size : total_size + (4 - remainder);
  return aligned_size;
}
...
static void *split_and_mark_used(struct heap *h, void *block_start, int needed_size) {
  ...
  int free_space = block_size - needed_size;
  if (free_space >= 3*HEADER_SIZE) {
    // split
    set_block_header(block_start, needed_size, 1);
    void *next_block = get_next_block(block_start);
    set_block_header(next_block, block_size - needed_size, 0);
...
void myfree(struct heap *h, void *payload) {
  ...
  void * previous_block = get_previous_block(block_start);
  if (is_within_heap_range(h, previous_block)) {
    if (!block_is_in_use(previous_block)) {
      coalesce(h, previous_block);
    }
  }
...
void *mymalloc(struct heap *h, unsigned int user_size) {
  ...
  void * curr = h->start;
  while (is_within_heap_range(h, curr)) {
    if (!block_is_in_use(curr) && get_block_size(curr) >= block_size) {
      // assign
      return split_and_mark_used(h, curr, block_size);
    } else {
