...
void fetch()
{
  ...
  {
    insOpExt = mem[pc] << 24 | mem[pc + 1] << 16 | mem[pc + 2] << 8 | mem[pc + 3]; // memory address or value
    pc += 4;
  }
}
...
int exec()
{
  ...
  // iocSwitch begin
  static const void *iocJumpTable[] = {
      &&C0, &&C1, &&C2, &&C3, &&C4,
      &&defaultCase, &&C6, &&C7, &&C8, &&C9,
      &&C10, &&C11, &&C12, &&defaultCase, &&defaultCase, &&C15};

  static const void *aluJumpTable[] = {
      &&A0, &&A1, &&A2, &&A3,
      &&A4, &&A5, &&A6, &&A7};
